...
# --- From distance_matrix.go ---
# (These functions remain the same, they work with ints)
...
def read_nodes(file_path: str) -> List[Node]:
    ...
    try:
        with open(file_path, mode='r', encoding='utf-8') as file:
            ...
            # MODIFIED: Removed the line that skips the header
            ...
            for i, row in enumerate(reader):
                ...
                # MODIFIED: Expecting 3 columns
                if len(row) < 3:
                    logging.warning(f"Skipping malformed row {row_num}: {row}")
                    continue

                try:
                    # MODIFIED: Map 3 columns and generate ID
                    node = Node(
